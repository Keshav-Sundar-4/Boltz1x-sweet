...
class Boltz1(LightningModule):
    ...
    def _configure_parameter_grads(self) -> None:
        ...
        print("[Model Grad Config] Fine-tuning Mode: Freezing MSA module only.")

        # Default: all parameters are trainable.
        for param in self.parameters():
            param.requires_grad = True

        # Freeze the MSA module as it's not used in this fine-tuning stage.
        if hasattr(self, 'msa_module') and self.msa_module is not None:
            print("[Model Grad Config] Freezing MSAModule parameters...")
            for param in self.msa_module.parameters():
                param.requires_grad = False
        else:
            print("[Model Grad Config] No MSAModule found to freeze ")

        # Summarize parameter status.
        ...
        # Sanity check for training mode.
    ...
    def forward(
        ...
        with torch.set_grad_enabled(
            ...
            # 1. Initialize the base sequence and pairwise embeddings
            ...
            for i in range(recycling_steps + 1):
                with torch.set_grad_enabled(self.training and (i == recycling_steps)):
                    ...
                    # Apply recycling using the (potentially refined) s_init and z_init
                    s = s_init + self.s_recycle(self.s_norm(s))
                    z = z_init + self.z_recycle(self.z_norm(z))

                    # MSA module refines the pairwise representation
                    if not self.no_msa:
                    ...
                    # Main pairformer refines the specialist-updated representations
        ...
        if self.confidence_prediction:
            ...
            detached_feats_dict = detach_feats(feats)
