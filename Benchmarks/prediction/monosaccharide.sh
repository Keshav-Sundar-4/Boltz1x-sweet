...
#SBATCH -p mrsec-gpu
#SBATCH --qos=medium
#SBATCH --gres=gpu:TitanXP:4
#SBATCH --job-name=Boltz_Glycan_Predict

# 1. Setup Environment and Directories
source activate /work/keshavsundar/env/boltz1x

TEMP_DIR="tmp_boltz_workspace"
INPUT_DIR="${TEMP_DIR}/input_yamls"
FINAL_DIR="monosaccharide_benchmark_Boltz_Sweet"
FIND_DIR="${TEMP_DIR}/boltz_results_input_yamls/predictions"

mkdir -p "$INPUT_DIR"
mkdir -p "$FINAL_DIR"
mkdir -p "$FIND_DIR" 

# 2. Embedded Python Script: Parse, Cross-Reference, and Generate YAMLs
echo "Running Python to cross-reference lists and generate YAMLs..."
python3 - << 'EOF'
import os
...
def parse_txt(filename, is_alpha):
    if not os.path.exists(filename): return
    with open(filename, 'r') as f:
        for line in f:
            target = '- alpha:' if is_alpha else '- beta:'
            if target in line:
                items = line.split(target)[1].split(',')
                for item in items:
                    item = item.strip()
                    if item:
                        alpha_set.add(item) if is_alpha else beta_set.add(item)

parse_txt('saccharides.txt', True)
parse_txt('saccharides.txt', False)
parse_txt('anomers.txt', True)
parse_txt('anomers.txt', False)

input_dir = 'tmp_boltz_workspace/input_yamls'

def write_yaml(code, suffix):
    if suffix in ['a', 'b']:
        iupac = f"{code}({suffix})"
        safe_id = f"{code}_{suffix}"
    else:
        iupac = code
        safe_id = f"{code}_other"
        
    with open(f"{input_dir}/{safe_id}.yaml", 'w') as f:
        f.write(f"version: 1\nsequences:\n  - glycan:\n      id: A\n      iupac: '{iupac}'\n")

count = 0
for g in ALLOWED_GLYCANS:
    found = False
    if g in alpha_set:
        write_yaml(g, 'a')
        found = True
...
print(f"Successfully generated {count} YAML files.")
EOF

# 3. BACKGROUND WATCHER: Move, Rename, and Clean on the fly
watch_and_move() {
    while true; do
        for filepath in "$FIND_DIR"/*/*.cif; do
            if [ -f "$filepath" ]; then
                filename=$(basename "$filepath")
                parent_dir=$(dirname "$filepath")
                
                # Because we used "id: A", Boltz names it A_model_0.cif. 
                # We must grab the actual molecule name from the parent directory instead.
                folder_name=$(basename "$parent_dir")
                
                # Convert safe ID back to requested format: _a -> (a), _other -> (other)
                final_name=$(echo "$folder_name" | sed 's/_a$/(a)/; s/_b$/(b)/; s/_other$/(other)/')
                
                # Move to final dir
                mv "$filepath" "${FINAL_DIR}/${final_name}.cif"
                
                # Instantly delete the prediction subfolder
...
# 4. Run Boltz Prediction (The Fix is right here)
echo "Starting Boltz prediction on 4 GPUs with --no_potentials..."

# Tricking PyTorch Lightning by hiding SLURM
TEMP_SLURM_ID=$SLURM_JOB_ID
unset SLURM_JOB_ID

# Removed 'srun' so the script handles the processing queue safely natively
boltz predict "$INPUT_DIR" \
  --out_dir "$TEMP_DIR" \
  --cache /work/keshavsundar/env/boltz1x/weights \
  --checkpoint /work/keshavsundar/env/boltz1x/weights/boltz1_glycan_epoch_1.ckpt \
  --devices 4 \
  --no_potentials

# Restore SLURM variable just in case the environment needs it later
export SLURM_JOB_ID=$TEMP_SLURM_ID

# 5. Cleanup & Final Sweep
echo "Boltz prediction complete. Stopping watcher..."
...
echo "Removing leftover temporary workspace..."
rm -rf "$TEMP_DIR"

echo "Done! All files successfully processed into $FINAL_DIR."
