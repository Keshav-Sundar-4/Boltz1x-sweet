...
#SBATCH -p guest-gpu
#SBATCH --qos=low-gpu
#SBATCH --job-name=AF3_Glycan_Predict
...
module purge
module load share_modules/APPTAINER/1.4.1

export TEMP_DIR="tmp_af3_workspace"
export INPUT_DIR="${TEMP_DIR}/input_jsons"
export OUTPUT_DIR="${TEMP_DIR}/af3_outputs"
export FINAL_DIR="monosaccharide_benchmark_AF3_Sweet"

mkdir -p "$INPUT_DIR"
mkdir -p "$OUTPUT_DIR"
mkdir -p "$FINAL_DIR"

# 2. Embedded Python Script: Generate a directory of individual JSON files
echo "Running Python to generate AlphaFold3 JSON inputs..."
python3 - << 'EOF'
import json
import os
...
for code in ALLOWED_GLYCANS:
    ...
    file_path = os.path.join(input_dir, f"{code}.json")
    with open(file_path, "w") as f:
        json.dump(job, f, indent=2)
    count += 1

print(f"Successfully generated {count} individual JSON files.")
EOF

# 3. BACKGROUND WATCHER: Move, Rename, and Clean on the fly
watch_and_move() {
    while true; do
        for filepath in "$OUTPUT_DIR"/*/*.cif; do
            if [ -f "$filepath" ]; then
...
# 4. Run AlphaFold3 Prediction via Apptainer (using --input_dir)
echo "Starting AlphaFold3 prediction..."

apptainer exec \
  --nv \
  --bind /work:/work \
  /work/yanghj/AF3/images/alphafold3.sif \
  python /app/alphafold/run_alphafold.py \
  --model_dir=/work/yanghj/AF3/models \
  --db_dir=/work/yanghj/AF3/db \
  --input_dir="$INPUT_DIR" \
  --output_dir="$OUTPUT_DIR"

# 5. Cleanup & Final Sweep
echo "AlphaFold3 prediction complete. Stopping watcher..."
