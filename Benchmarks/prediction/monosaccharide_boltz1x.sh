...
source activate /work/keshavsundar/env/original_boltz

CHECKPOINT="/work/keshavsundar/env/original_boltz/weights/boltz1_conf.ckpt"
CACHE_DIR="/work/keshavsundar/env/original_boltz/weights"
...
# 2. Embedded Python Script: Generate YAMLs directly from the 3-letter residue codes
echo "Running Python to generate YAMLs..."
python3 - << 'EOF'
import os
...
def write_yaml(code):
    path = os.path.join(input_dir, f"{code}.yaml")
...
for code in ALLOWED_GLYCANS:
    write_yaml(code)

print(f"Successfully generated {len(ALLOWED_GLYCANS)} YAML files.")
EOF
    ...
    while true; do
        for filepath in "$FIND_DIR"/*/*.cif; do
            if [ -f "$filepath" ]; then
                ...
                mv "$filepath" "${FINAL_DIR}/${folder_name}.cif"
